...
- Keep a dodge for roughly 100 ms while rechecking safety on every fresh packet. New predicted impacts, walls, or substantial losses of clearance override that commitment immediately. Small score differences need a larger advantage to reverse direction or toggle Shift. Repeated reads of the same packet keep the current command.
...
- Continue fitting latency when auras are distant; skip samples only when the player or aura could move into range during the measured interval.
